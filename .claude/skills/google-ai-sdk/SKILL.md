---
name: google-ai-sdk
description: Use for Google AI SDK, Gemini, structured outputs, tool calling, multimodal generation, streaming, safety settings, and AI provider integration work.
---
...
Before implementing, fetch the latest SDK docs:
- `@google/genai` → resolve library ID, query for `generateContent`, image generation, `responseModalities`
- `@ai-sdk/google` → resolve for `createGoogleGenerativeAI`, model IDs, `thinkingConfig`, `thought_signature`
- `ai` (Vercel AI SDK) → resolve for `generateText`, `streamText`, `generateObject`, `tool()`, `stopWhen`
