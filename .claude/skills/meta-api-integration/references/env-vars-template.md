...
VITE_META_APP_ID=                   # Unified Meta App ID for frontend
VITE_META_GRAPH_API_VERSION=v24.0   # Graph API version (must align with backend)
VITE_META_FALLBACK_REDIRECT_URI=    # Fallback redirect URI for OAuth popup failures

# --- Google Ads (separate OAuth) ---
GOOGLE_ADS_CLIENT_ID=               # Google Cloud OAuth Client ID
GOOGLE_ADS_CLIENT_SECRET=           # Google Cloud OAuth Client Secret
GOOGLE_ADS_REDIRECT_URI=            # OAuth callback URL for Google Ads
GOOGLE_ADS_DEVELOPER_TOKEN=         # Google Ads API developer token
GOOGLE_ADS_LOGIN_CUSTOMER_ID=       # Manager account Customer ID (MCC)
GOOGLE_ADS_API_VERSION=v18          # Google Ads API version
```
...
| `VITE_META_APP_ID` | ✅ | Unified Meta App ID (NeonDash convention) |
| `VITE_META_GRAPH_API_VERSION` | ⚠️ | Graph API version for JS SDK — must align with backend `META_GRAPH_API_VERSION` |
| `VITE_META_FALLBACK_REDIRECT_URI` | ⚠️ | Fallback redirect URI when JS SDK popup is blocked |

### Google Ads API

| Variable | Required | Description |
|----------|----------|-------------|
| `GOOGLE_ADS_CLIENT_ID` | ✅ | Google Cloud OAuth Client ID (Web application type) |
| `GOOGLE_ADS_CLIENT_SECRET` | ✅ | Google Cloud OAuth Client Secret |
| `GOOGLE_ADS_REDIRECT_URI` | ✅ | OAuth callback URL, e.g., `https://neondash.com.br/api/google-ads/callback` |
| `GOOGLE_ADS_DEVELOPER_TOKEN` | ✅ | API developer token from Google Ads Manager Account |
| `GOOGLE_ADS_LOGIN_CUSTOMER_ID` | ⚠️ | Manager account (MCC) Customer ID for acting on behalf of sub-accounts |
| `GOOGLE_ADS_API_VERSION` | ⚠️ | Google Ads API version (default: `v18`) |
...
FACEBOOK_ADS_REDIRECT_URI=https://yourdomain.com/api/facebook-ads/callback

# --- Frontend ---
VITE_META_APP_ID=${META_APP_ID}
VITE_META_GRAPH_API_VERSION=${META_GRAPH_API_VERSION}
VITE_META_FALLBACK_REDIRECT_URI=https://yourdomain.com/configuracoes/integrations/instagram
VITE_EMBEDDED_SIGNUP_CONFIG_ID=config_123...

# --- Google Ads (separate credentials) ---
GOOGLE_ADS_CLIENT_ID=xxx.apps.googleusercontent.com
GOOGLE_ADS_CLIENT_SECRET=GOCSPX-...
GOOGLE_ADS_REDIRECT_URI=https://yourdomain.com/api/google-ads/callback
