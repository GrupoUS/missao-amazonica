...
## Facebook JS SDK — "JSSDK Unknown Host domain"

**Symptom:** Console error "O domínio do host JSSDK é desconhecido" (JSSDK Unknown Host domain)

**Cause:** The domain hosting the Facebook SDK is not registered in the Meta App Dashboard.

**Fix:**
1. Go to [developers.facebook.com](https://developers.facebook.com) → Select your app
2. Left sidebar → **Use Cases** → **Authentication and account creation**
3. Click **Customize** → **Settings** (under Facebook Login)
4. Scroll to **"Allowed Domains for the JavaScript SDK"**
5. Add ALL domains where the SDK runs:
   - `http://localhost:5173` (Vite dev server)
   - `http://localhost:3000` (API server)
   - `https://neondash.gpus.com.br` (production)
6. Also verify **Settings → Basic → "App Domains"** includes: `localhost`, `neondash.gpus.com.br`
7. **Save Changes**

> **Note:** Changes take effect immediately. No app restart required.

---

## Console Errors from Ad-Blockers (False Positives)

**Symptom:** Multiple console errors that appear unrelated to application code:
- `Failed to apply filter` / `Invalid flags supplied to RegExp constructor`
- `ERR_BLOCKED_BY_CLIENT` on `facebook.com/platform/impression.php`
- `Failed to load resource` for Facebook tracking URLs

**Cause:** Browser ad-blocker extensions (uBlock Origin, AdBlock Plus, etc.) injecting filter lists that conflict with Facebook SDK scripts and block tracking pixels.

**Verification:** Disable the ad-blocker extension temporarily. If errors disappear, they are ad-blocker artifacts.

**Impact:** None. These errors do NOT affect SDK functionality, login flows, or API calls. The Facebook SDK degrades gracefully when tracking pixels are blocked.
